...
def init_db() -> None:
    ...
    Table definitions (SQLModel) and seed rows (schema.py constants) are the same
    single source the Alembic migrations build from, so this only offers a second
    *application path* for local runs — never a second copy of the data.
    """
    ...
    with Session(engine) as session:
        ...
        if session.exec(select(Departments)).first() is None:
            session.add_all(Departments(**row) for row in DEPARTMENTS)
        session.commit()

...
down_revision: str | Sequence[str] | None = "a96c2c884ccb"
...
def downgrade() -> None:
    ...
    op.execute(
        departments_per_floor_table.delete().where(
            departments_per_floor_table.c.floor.in_([row["floor"] for row in DEPARTMENTS_PER_FLOOR])
        )
    )
    op.execute(
        mentors_for_interns_table.delete().where(
            mentors_for_interns_table.c.name.in_([row["name"] for row in MENTORS_FOR_INTERNS])
        )
    )
